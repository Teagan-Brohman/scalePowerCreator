...
Examples:
  ...
  # Calculate mass for specific volume and density
  python isotope_calculator.py U3Si2 --volume 18.25 --density 12.2
  
  # Calculate individual isotope masses from total mass
  python isotope_calculator.py UO2 --mass 100.0
        """)
    ...
    parser.add_argument('--volume', type=float, metavar='CM3',
                       help='Volume in cm³ to calculate mass (requires --density)')
    parser.add_argument('--density', type=float, metavar='G_CM3',
                       help='Density in g/cm³ to calculate mass (requires --volume)')
    parser.add_argument('--mass', type=float, metavar='GRAMS',
                       help='Total mass in grams to calculate individual isotope masses')
    
    args = parser.parse_args()
    
    # Validate mass calculation arguments
    if args.volume is not None and args.density is None:
        print("Error: --volume requires --density")
        sys.exit(1)
    if args.density is not None and args.volume is None:
        print("Error: --density requires --volume") 
        sys.exit(1)
    if args.mass is not None and (args.volume is not None or args.density is not None):
        print("Error: --mass cannot be used with --volume/--density")
        sys.exit(1)
    ...
    try:
        ...
        # Calculate mass information if requested
        total_mass = None
        isotope_masses = {}
        
        if args.volume is not None and args.density is not None:
            total_mass = args.volume * args.density
        elif args.mass is not None:
            total_mass = args.mass
            
        if total_mass is not None:
            for isotope_name, fraction in fractions.items():
                isotope_masses[isotope_name] = fraction * total_mass
        ...
        output_data = {
            ...
            'volume_cm3': args.volume,
            'density_g_cm3': args.density,
            'total_mass_g': total_mass,
            'isotope_masses_g': isotope_masses if isotope_masses else None
        }
        ...
        if args.output:
            ...
            if total_mass is not None:
                print(f"{'Isotope':>8} {'Weight %':>10} {'Mass (g)':>12}")
                print("-" * 50)
                for isotope_name, fraction in sorted_fractions:
                    mass_g = isotope_masses[isotope_name]
                    print(f"{isotope_name:>8}: {fraction*100:8.3f}% {mass_g:11.6f} g")
                print("-" * 50)
                print(f"{'Total:':>8} {sum(fractions.values())*100:8.3f}% {total_mass:11.6f} g")
                
                if args.volume is not None and args.density is not None:
                    print(f"\nCalculation Details:")
                    print(f"  Volume: {args.volume:.6f} cm³")
                    print(f"  Density: {args.density:.3f} g/cm³")
                    print(f"  Total Mass: {total_mass:.6f} g")
            else:
                for isotope_name, fraction in sorted_fractions:
