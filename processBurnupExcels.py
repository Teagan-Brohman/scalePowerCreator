...
Usage:
    python processBurnupExcels.py

Requirements:
    - excelStorage/Burnup.xlsx (main Excel file)
    - excelStorage/BURNUPROM2006-1992.xlsm (secondary Excel file)
...
class BurnupProcessor:
    def __init__(self, main_excel="excelStorage/Burnup.xlsx", secondary_excel="excelStorage/BURNUPROM2006-1992.xlsm", 
    ...
    def normalize_dataframe_columns(self, df, sheet_name, source):
        ...
        column_mapping = {
            ...
            'Time @': ['Time @', 'At'],
            'Time @_1': ['Time @_1', 'Time @ _1', 'Time @.1', '@'],
            'Time D': ['Time D', 'Down', 'Time Delta'],
            'Time D_1': ['Time D_1', 'D', 'Time Delta '],
            'Shutdown\nTime': ['Shutdown\nTime', 'Shutdown Time', 'Shutdown'],
            'Delta Time\n(minutes)': ['Delta Time\n(minutes)', 'Delta Time (minutes)', 'Delta Time(minutes)', 'Delta Time', 'Delta Time (Minutes)'],
            'Power\nDuration': ['Power\nDuration', 'Power Duration', 'Duration'],
            'Power\nCycles': ['Power\nCycles', 'Power Cycles', 'Cycles'],
            'Checkout\nStart_1': ['Checkout\nStart_1', 'Checkout Start_1', 'Checkout Start'],
        ...
        for std_col, possible_names in column_mapping.items():
            for possible_name in possible_names:
                if possible_name in df.columns:
                    mapped_original_cols.add(possible_name)
                    break
        
        unmapped_cols = [col for col in df.columns if col not in mapped_original_cols and col not in ['start_year', 'end_year', 'sheet_name', 'original_row', 'source_file']]
        if unmapped_cols:
            print(f"    ⚠ Unmapped columns in {sheet_name}: {unmapped_cols}")
            ...
            try:
                if sheet_type == 'standard':
                    ...
                    year_mappings = {
                        ...
                        '04-02-01 to 09-03-01': (2001, 2001),  # Updated
                        '10-01-02 to 03-31-03': (2002, 2003),
                        '10-02-00 to 4-3-00': (2000, 2000),  # New
                        '10-05-01 to 03-28-02': (2001, 2002),  # Updated
                        '10-1-1998 to 3-31-99': (1998, 1999),
                        '10-1-92 to 4-31-93': (1992, 1993),
                        '4-1-99 to 3-26-00': (1999, 2000),  # New
                        '4-1-99 to 4-3-00 - In Progress': (1999, 2000),  # New with suffix
        ...
        try:
            ...
            cursor.execute('''
                ...
                SET power_per_minute_avg = 
                    CASE 
                        -- Handle zero Power Duration with valid Delta Time
                        WHEN (`Power\nDuration` = 0 OR `Power\nDuration` = '0.0')
                             AND `Total Energy\n(average)` IS NOT NULL
                             AND `Delta Time\n(minutes)` IS NOT NULL
                             AND CAST(`Delta Time\n(minutes)` AS REAL) > 0
            ...
            # Check for NULL power_per_minute values and report why
            cursor.execute('''
                SELECT COUNT(*) as total,
                       COUNT(CASE WHEN (`Power\nDuration` = 0 OR `Power\nDuration` = '0.0') 
                                   AND (`Delta Time\n(minutes)` IS NULL OR CAST(`Delta Time\n(minutes)` AS REAL) <= 0) 
                                  THEN 1 END) as zero_duration_no_delta,
                       COUNT(CASE WHEN CAST(`Power\nDuration` AS REAL) < 0 THEN 1 END) as negative_duration,
                       COUNT(CASE WHEN `Power\nDuration` IS NULL THEN 1 END) as null_duration
                FROM burnup_data 
                WHERE power_per_minute_avg IS NULL
            ''')
            null_stats = cursor.fetchone()
            
            # Also get count of zero duration cases that were successfully calculated
            cursor.execute('''
                SELECT COUNT(*) as zero_duration_calculated
                FROM burnup_data 
                WHERE (`Power\nDuration` = 0 OR `Power\nDuration` = '0.0')
                      AND power_per_minute_avg IS NOT NULL
            ''')
            zero_calculated = cursor.fetchone()[0]
            
            if null_stats[0] > 0:
                print(f"\n⚠ WARNING: {null_stats[0]} rows have NULL power_per_minute values:")
                print(f"  - Power Duration = 0 (no valid Delta Time): {null_stats[1]} rows")
                print(f"  - Power Duration < 0: {null_stats[2]} rows (invalid negative duration)")
                print(f"  - Power Duration is NULL: {null_stats[3]} rows (missing data)")
                
            if zero_calculated > 0:
                print(f"\n✓ RECOVERED: {zero_calculated} rows with zero Power Duration calculated using Total Energy / Delta Time")
                ...
                cursor.execute('''
                    ...
                    LIMIT 5
                ''')
                print(f"  Most affected years:")
                for year, count in cursor.fetchall():
                    print(f"    - {year}: {count} rows")
                
                # Show specific dates for each error type
                print(f"\n  Specific dates with Power Duration errors:")
                
                # Zero duration dates (without valid Delta Time)
                if null_stats[1] > 0:
                    cursor.execute('''
                        SELECT COALESCE(Date, datetime_combined) as effective_date, start_year, sheet_name, `Power\nDuration`, `Delta Time\n(minutes)`
                        FROM burnup_data 
                        WHERE power_per_minute_avg IS NULL 
                          AND (`Power\nDuration` = 0 OR `Power\nDuration` = '0.0')
                          AND (`Delta Time\n(minutes)` IS NULL OR CAST(`Delta Time\n(minutes)` AS REAL) <= 0)
                        ORDER BY start_year, effective_date
                        LIMIT 10
                    ''')
                    print(f"    Zero Power Duration - no valid Delta Time ({null_stats[1]} total, showing first 10):")
                    for date, year, sheet, duration, delta_time in cursor.fetchall():
                        date_str = date if date else "No date available"
                        delta_str = f", delta_time: {delta_time}" if delta_time is not None else ", delta_time: NULL"
                        print(f"      - {year}: {date_str} (sheet: {sheet}, duration: {duration}{delta_str})")
                
                # Show recovered zero duration cases
                if zero_calculated > 0:
                    cursor.execute('''
                        SELECT COALESCE(Date, datetime_combined) as effective_date, start_year, sheet_name, `Power\nDuration`, `Delta Time\n(minutes)`, power_per_minute_avg
                        FROM burnup_data 
                        WHERE (`Power\nDuration` = 0 OR `Power\nDuration` = '0.0')
                          AND power_per_minute_avg IS NOT NULL
                        ORDER BY start_year, effective_date
                        LIMIT 5
                    ''')
                    print(f"    Zero Power Duration - successfully calculated ({zero_calculated} total, showing first 5):")
                    for date, year, sheet, duration, delta_time, power_avg in cursor.fetchall():
                        date_str = date if date else "No date available"
                        print(f"      - {year}: {date_str} (sheet: {sheet}, power: {power_avg} MW/min from {delta_time} min)")
                
                # Negative duration dates  
                if null_stats[2] > 0:
                    cursor.execute('''
                        SELECT COALESCE(Date, datetime_combined) as effective_date, start_year, sheet_name, `Power\nDuration`
                        FROM burnup_data 
                        ...
                        ORDER BY start_year, effective_date
                    ''')
                    print(f"    Negative Power Duration ({null_stats[2]} total):")
                    for date, year, sheet, duration in cursor.fetchall():
                        date_str = date if date else "No date available"
                        print(f"      - {year}: {date_str} (sheet: {sheet}, duration: {duration})")
                
                # NULL duration dates
                if null_stats[3] > 0:
                    cursor.execute('''
                        SELECT COALESCE(Date, datetime_combined) as effective_date, start_year, sheet_name
                        FROM burnup_data 
                        WHERE power_per_minute_avg IS NULL 
                          AND `Power\nDuration` IS NULL
                        ORDER BY start_year, effective_date
                    ''')
                    print(f"    NULL Power Duration ({null_stats[3]} total):")
                    for date, year, sheet in cursor.fetchall():
                        date_str = date if date else "No date available"
                        print(f"      - {year}: {date_str} (sheet: {sheet})")
    ...
    def generate_final_report(self):
        ...
        try:
            ...
            cursor.execute('''
                ...
                WHERE power_per_minute_avg IS NULL
            ''')
            total_nulls = cursor.fetchone()[0]
            
            if total_nulls > 0:
                print(f"\nData Quality Issues:")
                print(f"  - Rows with invalid Power Duration: {total_nulls}")
                print(f"    (These rows have NULL power_per_minute values due to Excel data anomalies)")
            else:
                print(f"\nData Quality: ✅ All power calculations successful")
